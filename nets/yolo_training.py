...
class IOUloss(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        if self.loss_type == "iou":
            ...
            cw = torch.max(target_x2y2[:, 0], pred_x2y2[:, 0]) - torch.min(target_x1y1[:, 0], pred_x1y1[:, 0])
            ch = torch.max(target_x2y2[:, 1], pred_x2y2[:, 1]) - torch.min(target_x1y1[:, 1], pred_x1y1[:, 1])
            c2 = cw ** 2 + ch ** 2 + eps
            rho2 = ((target_x1y1[:, 0] + target_x2y2[:, 0] - pred_x1y1[:, 0] - pred_x2y2[:, 0]) ** 2 +
                    (target_x1y1[:, 1] + target_x2y2[:, 1] - pred_x1y1[:, 1] - pred_x2y2[:, 1]) ** 2) / 4
            w1, h1 = pred_x2y2[:, 0] - pred_x1y1[:, 0], pred_x2y2[:, 1] - pred_x1y1[:, 1] + eps
            w2, h2 = target_x2y2[:, 0] - target_x1y1[:, 0], target_x2y2[:, 1] - target_x1y1[:, 1] + eps
...
class YOLOLoss(nn.Module):
    def __init__(self, num_classes, fp16, iou_type, strides=[8, 16, 32]):   # 下采样尺度已经设置好了
        super().__init__()
    ...
    def get_output_and_grid(self, output, k, stride):
        ...
        # 中心点偏移量未采用sigmoid激活，这和SAMOTA匹配机制有关。
        output[..., :2] = (output[..., :2] + grid.type_as(output)) * stride  # 原图上的尺度
    ...
    def get_losses(self, x_shifts, y_shifts, expanded_strides, labels, outputs):
        ...
        for batch_idx in range(outputs.shape[0]):
            ...
            if num_gt == 0:
                ...
                num_fg += num_fg_img
                # 预测框和gt的iou越大，那么我们希望预测得到的置信度越大，反之亦然
            ...
            cls_targets.append(cls_target)  # 保存每张图片匹配的anchor对应的gt类别
            reg_targets.append(reg_target)  # 保存每张图片匹配的anchor对应的gt坐标(xywh)
            obj_targets.append(obj_target.type(cls_target.type()))  # 保存每张图片匹配的anchor的mask，转成int类型
        ...
        loss_obj = (self.bcewithlog_loss(obj_preds.view(-1, 1), obj_targets)).sum()  # obj损失，正负样本都要计算，不用fg_mask
        loss_cls = (self.bcewithlog_loss(cls_preds.view(-1, self.num_classes)[fg_masks], cls_targets)).sum()
        reg_weight = 5.0
        loss = reg_weight * loss_iou + loss_obj + loss_cls

        return loss / num_fg  # 除以所有图片的正样本数量之和
    ...
    def get_assignments(self, num_gt, total_num_anchors, gt_bboxes_per_image, gt_classes, bboxes_preds_per_image,
        ...
        #   is_in_boxes_and_center  [num_gt, sum(fg_mask)]
        # -------------------------------------------------------#
        ...
        # -------------------------------------------------------#
        # fg_mask : 你这个特征点要么至少被一个gt覆盖，要么至少被一个fixed_center_area覆盖 （8400）
        # 根据fg_mask过滤特征点
        ...
        # 计算iou_loss矩阵  （num_gt, num_anchor_points）
    ...
    def get_in_boxes_info(self, gt_bboxes_per_image, expanded_strides, x_shifts, y_shifts, total_num_anchors, num_gt,
        ...
        # -------------------------------------------------------#
        # 判断特征点中心(x_center, y_center)是否被gt覆盖，必须四个全大于等于0，该特征点才被gt覆盖
        ...
        # -------------------------------------------------------#
        #   is_in_boxes     [num_gt, n_anchors_all] ： 表示某特征点中心是否被某gt覆盖的mask
        #   is_in_boxes_all [n_anchors_all]         ： 表示某特征点中心至少被这张图片中的一个gt覆盖的mask
        # -------------------------------------------------------#
        is_in_boxes = bbox_deltas.min(dim=-1).values > 0.0
        is_in_boxes_all = is_in_boxes.sum(dim=0) > 0

        # x1_fixed_center_area
        ...
        # -------------------------------------------------------#
        # 判断特征点中心(x_center, y_center)是否被fixed_center_area覆盖，必须四个全大于等于0，该特征点才被fixed_center_area覆盖
        ...
        # -------------------------------------------------------#
        #   is_in_centers       [num_gt, n_anchors_all] ： 表示某特征点中心是否被某fixed_center_area覆盖的mask
        #   is_in_centers_all   [n_anchors_all]         ： 表示某特征点中心至少被这张图片中的一个fixed_center_area覆盖的mask
        # -------------------------------------------------------#
    ...
    def dynamic_k_matching(self, cost, pair_wise_ious, gt_classes, num_gt, fg_mask):
        ...
        # dynamic_ks：每个gt匹配的特征点数量
        dynamic_ks = torch.clamp(topk_ious.sum(1).int(), min=1)

        for gt_idx in range(num_gt):
            # ------------------------------------------------------------#
            #   给每个真实框选取loss最小的动态k个点
            # ------------------------------------------------------------#
        ...
        # ------------------------------------------------------------#
        # 当一个anchor匹配到多个gt时，将此anchor分配给loss最小的gt(一个anchor只能匹配一个gt，一个gt可以由多个anchor匹配)
        ...
        num_fg = fg_mask_inboxes.sum().item()  # 正样本anchor的个数
        ...
        pred_ious_this_matching = (matching_matrix * pair_wise_ious).sum(0)[fg_mask_inboxes]  # 正样本anchor和对应匹配的gt的iou
